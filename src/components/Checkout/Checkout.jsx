...
const Checkout = () => {
    ...
    const manejadorSubmit = (event) => {
        ...
        if (!nombre || !apellido || !telefono || !email || !emailConfimarcion) {
            setError("Por favor complete los datos pedidos");
            return;
        }

        //Se valida que coincidan los emails
        ...
        addDoc(collection(db, "ordenes"), orden)
            ...
            .catch((error) => {
                ...
                setError("Se produjo al comprar, vuelva a intentar");
            })
    ...
    return (
        <div>
            ...
            <form onSubmit={manejadorSubmit} className="formulario">
                ...
                <button className="button" type="submit"> Concretar compra </button>

                {
                    ordenId && (
                        <strong className="orderId">¡Ahora sos parte de la Narucomunidad! Gracias por tu compra! Tu número de orden es: {ordenId} </strong>
                    )
                }
            </form>
        </div>
